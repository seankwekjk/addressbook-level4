...
=== Removing a Tag from all contacts in the AddressBook : `removeTag`
...
* All instances of the tag will be removed from the AddressBook.
****

Examples:

* `removeTag friends` +
Searches throgugh all contacts in the AddressBook and deletes all instances of the Tag.
...
=== Locating Contacts by any other field: `find` `Coming in v2.0`
...
1. Auto-login for mail and auto-sending of mail without needing to click send on the mail application.
...
Subsequently, when they typed in the command free followed by the contact name or index, they and get a list of common free timings with that contact.
