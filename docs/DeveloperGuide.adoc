...
The listbirthday command is facilitated by the `BirthdayChecker` class in the person class. Take note that the listbirthday command is not an undoable command and hence inherits from `Command` rather than `UndoableCommand` (see design considerations below).
...
The following sequence diagram _(Figure 4.6a)_ illustrates the execution of the `sort` command in Contags.

image::SortCommandSequenceDiagram.png[width="800"]
_Figure 4.6a : SortCommand sequence diagram_
...
_Figure 4.8a : SMS Command sequence diagram_
