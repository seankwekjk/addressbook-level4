...
:repoURL: https://github.com/se-edu/Contags-level4/tree/master
...
After forking the repo, links in the documentation will still point to the `se-edu/Contags-level4` repo. If you plan to develop this as a separate product (i.e. instead of contributing to the `se-edu/Contags-level4`) , you should replace the URL in the variable `repoURL` in `DeveloperGuide.adoc` and `UserGuide.adoc` with the URL of your fork.
...
[NOTE]
Note how the `Model` simply raises a `ContagsChangedEvent` when the Address Book data are changed, instead of asking the `Storage` to save the updates to the hard disk.
...
** link:{repoURL}/src/main/java/seedu/address/logic/parser/ContagsParser.java[`ContagsParser`] is responsible for analyzing command words.
* Solution
** Modify the switch statement in link:{repoURL}/src/main/java/seedu/address/logic/parser/ContagsParser.java[`ContagsParser#parseCommand(String)`] such that both the proper command word and alias can be used to execute the same intended command.
** See this https://github.com/se-edu/Contags-level4/pull/590/files[PR] for the full solution.
****
...
**  Find out which of the existing API methods in  link:{repoURL}/src/main/java/seedu/address/model/Contags.java[`Contags`] and link:{repoURL}/src/main/java/seedu/address/model/person/Person.java[`Person`] classes can be used to implement the tag removal logic. link:{repoURL}/src/main/java/seedu/address/model/Contags.java[`Contags`] allows you to update a person, and link:{repoURL}/src/main/java/seedu/address/model/person/Person.java[`Person`] allows you to update the tags.
* Solution
** Add the implementation of `deleteTag(Tag)` method in link:{repoURL}/src/main/java/seedu/address/model/ModelManager.java[`ModelManager`]. Loop through each person, and remove the `tag` from each person.
** See this https://github.com/se-edu/Contags-level4/pull/591/files[PR] for the full solution.
****
...
* Solution
** See this https://github.com/se-edu/Contags-level4/pull/592/files[PR] for the full solution.
****
...
** See this https://github.com/se-edu/Contags-level4/pull/593/files[PR] for the full solution.
****
...
** Use link:{repoURL}/src/main/java/seedu/address/ui/StatusBarFooter.java[`StatusBarFooter#handleContagsChangedEvent(ContagsChangedEvent)`] to update the number of persons whenever there are new changes to the Contags.
** See this https://github.com/se-edu/Contags-level4/pull/596/files[PR] for the full solution.
****
...
. Add a new method `backupContags(ReadOnlyContags)`, so that the address book can be saved in a fixed temporary location.
+
****
* Hint
** Add the API method in link:{repoURL}/src/main/java/seedu/address/storage/ContagsStorage.java[`ContagsStorage`] interface.
** Implement the logic in link:{repoURL}/src/main/java/seedu/address/storage/StorageManager.java[`StorageManager`] class.
* Solution
** See this https://github.com/se-edu/Contags-level4/pull/594/files[PR] for the full solution.
****
...
. Modify link:{repoURL}/src/main/java/seedu/address/logic/parser/ContagsParser.java[`ContagsParser`] to accept a `RemarkCommand`.

**Tests:**

. Add `RemarkCommandTest` that tests that `executeUndoableCommand()` throws an Exception.
. Add new test method to link:{repoURL}/src/test/java/seedu/address/logic/parser/ContagsParserTest.java[`ContagsParserTest`], which tests that typing "remark" returns an instance of `RemarkCommand`.
...
. Modify link:{repoURL}/src/main/java/seedu/address/logic/parser/ContagsParser.java[`ContagsParser`] to use the newly implemented `RemarkCommandParser`.
...
. Modify link:{repoURL}/src/test/java/seedu/address/logic/parser/ContagsParserTest.java[`ContagsParserTest`] to test that the correct command is generated according to the user input.
...
. Modify link:{repoURL}/src/main/java/seedu/address/model/util/SampleDataUtil.java/[`SampleDataUtil`] to add remarks for the sample data (delete your `Contags.xml` so that the application will load the sample data when you launch it.)
...
. Fix `validContags.xml` such that the XML tests will not fail due to a missing `<remark>` element.
...
See this https://github.com/se-edu/Contags-level4/pull/599[PR] for the step-by-step solution.
...
|`* * *` |user |find a person with an incomplete name |to find a person even if I cannot spell the person's name perfectly
...
|`* * *` |user |create a new contact with a name that already exists in Contags |have multiple contacts with the same name

|`* *` |user |hide link:#private-contact-detail[private contact details] by default |minimize chance of someone else seeing them by accident

|`* *` |user |add contacts with missing fields |
...
|`* *` |user |sort the list of contacts in the list based on phone number |
...
|`*` |user |view the last 10 commands I typed |track use of the Contags
...
2.  Contags shows a list of persons
...
+
[none]
** 5a1. Contags shows an error message.
+
Use case ends.
...
2. Contags shows list of existing contacts of same name.
...
+
*** 3b11. Contags shows an error message.
+
...
+
Contags informs user that Person Index is invalid.

* 1b. User requests to edit information in an invalid format
+
Contags informs user that the format is invalid, providing the correct format.
...
*MSS*
 ...
 1.  User requests to find person by entering any keyword that is part of contact particulars
 2.  Contags finds contact with keyword included in either name, address, email, or phone number, and displays latest change
 +
 Use case ends.
...
+
Contags informs user that contact cannot be found
...
+
Contags informs user that there are currently no contacts in the list.

* 2a. User inputs invalid contact index.
+
Contags informs user that the input is not a valid contact index.
...
.  Data held in Contags cannot be corrupted in event of unexpected errors.
