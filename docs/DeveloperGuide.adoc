...
Contags is a platform for storing and managing contacts and their associated information. It provides you with a set of core APIs which you can use to concentrate on the production, editing and publishing of enhancements, new features and documentation.

In order to be a successful Contags developer, you need a general
understanding of: +
...
[NOTE]
Note how the `Model` simply raises a `AddressBookChangedEvent` when the Contags data are changed, instead of asking the `Storage` to save the updates to the hard disk.
...
**Cons:** As the `RemarkCommand` requires the input of an `index`, we require to interface with the list of persons already existing in the AddressBook. These methods however, return lists of `ReadOnlyPerson`, which do not allow the inclusion of a `setReamrk()` method. This would require changes elsewhere in the project that would affect other aspects.
...
|`* * *` |user |create a new contact with a name that already exists in addressbook |have multiple contacts with the same name
...
|`*` |user |view the last 10 commands I typed |track use of Contags
